...
// --- Constants & Types ---
const apiKey = ""; // In production, move to backend

// --- AI Service ---
const fetchGemini = async (prompt: string, systemInstruction = "") => {
...
const App = () => {
    ...
    // AI
    const handleAiConsult = async (topic: string) => {
        setIsAiLoading(true);
        setShowAiPanel(true);
        try {
            const system = "Ты — AI-консультант TauTan Secure. Отвечай кратко, профессионально, на русском языке.";
            const response = await fetchGemini(`Расскажи подробно про ${topic} в контексте безопасности аппаратных ключей.`, system);
            setAiResponse(response || null);
        } catch (e) {
            setAiResponse("Ошибка связи с ИИ. Попробуйте позже.");
        } finally {
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-grow glass-card p-8 rounded-[32px] relative overflow-hidden group border-zinc-800">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-zinc-400 max-w-md mb-8 leading-relaxed">Ваш аппаратный токен полностью функционален. Последняя проверка безопасности пройдена 12 минут назад.</p>
                        <div className="flex flex-wrap gap-4">
                            <button onClick={handleCheckUpdate} className="px-6 py-3 bg-brand-orange text-white font-bold rounded-full text-sm hover:scale-105 transition-transform active:scale-95 shadow-lg shadow-brand-orange/20">Проверить обновления</button>
                            <button onClick={() => handleAiConsult('безопасность аппаратных ключей')} className="px-6 py-3 border border-zinc-700 text-zinc-300 font-bold rounded-full text-sm hover:bg-zinc-800 transition-colors">AI Анализ</button>
                        </div>
                    </div>
                </div>
    ...
    const renderFidoView = () => (
        <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
            <div className="flex justify-between items-center px-2">
                ...
                <button onClick={() => setWizardStep(1)} className="px-6 py-2 bg-brand-orange text-white font-bold rounded-full text-sm flex items-center gap-2 hover:scale-105 transition-transform"><Fingerprint size={16} /> Регистрация</button>
            </div>
            {wizardStep === 0 ? (
        ...
        <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                    ...
                    <button onClick={handleCheckUpdate} className="w-full py-4 bg-brand-orange border border-brand-orange/20 text-white rounded-full text-xs font-bold hover:scale-105 transition-all uppercase tracking-widest shadow-[0_0_20px_rgba(238,93,67,0.3)]">Проверить обновление</button>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-zinc-100 font-sans selection:bg-brand-orange/30 flex relative overflow-x-hidden">
            ...
            <aside className={`fixed top-0 left-0 h-full sidebar-blur border-r border-white/5 z-40 main-transition flex flex-col ${isSidebarOpen ? 'w-64' : 'w-20'} hidden md:flex`}>
                <div className="p-6 mb-4 flex items-center justify-center">
                    <div className="flex items-center gap-3 group cursor-pointer w-full">
                        <div className="min-w-[40px] h-10 bg-brand-orange rounded-xl flex items-center justify-center shadow-[0_0_20px_rgba(238,93,67,0.3)] shrink-0">
                            <Shield className="text-white" size={24} />
                        </div>
                        {isSidebarOpen && (
                            <div className="animate-fade-in">
                                <span className="text-xl font-black tracking-tighter text-white">TAUTAN</span>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="p-6 border-t border-white/5 space-y-4">
                    <button className="flex items-center gap-4 text-zinc-500 hover:text-white transition-colors px-4 py-2 w-full">
            ...
            <div className={`flex-grow flex flex-col min-w-0 main-transition ${isSidebarOpen ? 'md:pl-64' : 'md:pl-20'}`}>
                <header className="md:hidden flex items-center justify-between p-5 sidebar-blur border-b border-white/5 sticky top-0 z-30">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold tracking-tight">TAUTAN</span>
                    </div>
                    ...
                    <div className="max-w-6xl mx-auto">
                        ...
                        {currentView === 'faq' && (
                            <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
                                ...
                                <button onClick={() => handleAiConsult('faq и частые вопросы')} className="px-8 py-4 bg-brand-orange rounded-full font-bold flex items-center gap-2 hover:scale-105 transition-transform"><Sparkles size={18} /> Спросить ИИ</button>
                            </div>
                        )}
                    </div>
                </main>

                <div className="w-full bg-brand-orange py-2 overflow-hidden select-none z-10 relative">
                    <div className="flex animate-marquee whitespace-nowrap">
                        {[...Array(12)].map((_, i) => (
                            <span key={i} className="mx-8 text-white text-[9px] font-black uppercase tracking-[0.3em] flex items-center gap-3">
                                PROTECTED BY TSARKA
                                <img src="/img/tsarka_bars_logo.png" alt="Tsarka Bars" className="h-12 w-auto opacity-90 brightness-0 invert grayscale" />
                            </span>
                        ))}
                    </div>
                </div>
            ...
            <div className={`fixed top-0 right-0 h-full w-full md:w-96 sidebar-blur z-50 transform transition-transform duration-500 ease-in-out border-l border-white/10 ${showAiPanel ? 'translate-x-0 shadow-[-40px_0_60px_rgba(0,0,0,0.8)]' : 'translate-x-full'}`}>
                <div className="h-full flex flex-col">
                    <div className="p-6 border-b border-white/5 flex justify-between items-center bg-zinc-900/20">
                        <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-lg bg-brand-orange/10 flex items-center justify-center text-brand-orange"><Sparkles size={16} /></div>
                            <span className="font-bold text-xs tracking-widest uppercase">AI Expert</span>
                        </div>
                        <button onClick={() => setShowAiPanel(false)} className="p-2 hover:bg-white/5 rounded-full transition-colors"><X size={20} /></button>
                    </div>
                    <div className="flex-grow overflow-y-auto p-8 space-y-6 scrollbar-hide">
                        {isAiLoading ? (
                            <div className="flex flex-col items-center justify-center h-full space-y-4">
                                ...
                                <p className="text-[10px] text-zinc-600 uppercase tracking-[0.2em] animate-pulse">Анализ безопасности...</p>
                            </div>
                        ) : (
                            <div className="animate-slide-in-right">
                                <div className="p-6 bg-zinc-900/60 border border-white/5 rounded-3xl mb-4">
                                    <p className="text-sm leading-relaxed text-zinc-300 whitespace-pre-line">{aiResponse || "Привет! Я эксперт по безопасности TauTan. Как я могу помочь?"}</p>
                                </div>
                            </div>
                        )}
                    </div>
                    <div className="p-6 border-t border-white/5 text-center"><p className="text-[8px] text-zinc-700 uppercase tracking-widest">TauTan Secure AI Engine v2.5</p></div>
                </div>
            </div>
        </div>
    );
